...
> **Diagram notes:** The flow goes — Client browsers → Vercel CDN (static hosting) → Express API (Middleware → Routes → Controllers) → Redis cache (cache-hit path) / MongoDB Atlas (cache-miss path). External services **Cloudinary** (image storage) and **Razorpay** (payment gateway) are also called from the Controllers layer.
